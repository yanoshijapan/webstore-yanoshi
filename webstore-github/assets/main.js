...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getTotalQuantityForPrefix(cart, prefix) { let totalQuantity = 0; if (!cart || !Array.isArray(cart)) return 0; cart.forEach(item => { if (item && typeof item.id === 'string' && item.id.startsWith(prefix)) { totalQuantity += item.quantity; } }); return totalQuantity; }
  function getTieredPrice(item, totalQuantityForTier) { if (!item || typeof item.id !== 'string') return item.price || 0; const productIdPrefix = item.id.split('-')[0].substring(0, 2); const rule = PRICING_RULES[productIdPrefix]; if (rule) { for (const tier of rule.tiers) { if (totalQuantityForTier >= tier.minQty) { return tier.price; } } } return item.price; }
  window.addToCartFromDynamic = (buttonElement, productId) => { const productCard = buttonElement.closest('.product-item-card'); if (!productCard) { console.error('Tidak dapat menemukan kartu produk.'); return; } const name = productCard.querySelector('.product-name').textContent; const image = productCard.querySelector('.product-image').src; const priceString = productCard.querySelector('.product-price').textContent; const price = parseInt(priceString.replace(/[^0-9]/g, ''), 10); const sizeSelector = productCard.querySelector('.size-selector'); let selectedSize = ''; if (sizeSelector) { selectedSize = sizeSelector.value; } const cartItemId = selectedSize && selectedSize !== '-' ? productId + '-' + selectedSize : productId; const nameForCart = selectedSize && selectedSize !== '-' ? name + ' - Size ' + selectedSize : name; let cart = getCart(); const existingItem = cart.find(item => item.id === cartItemId); if (existingItem) { existingItem.quantity += 1; } else { cart.push({ id: cartItemId, name: nameForCart, price: price, image: image, quantity: 1 }); } saveCart(cart); showToast(nameForCart + ' ditambahkan'); };
  function updateCartIcon() { const cart = getCart(); const cartCountEl = document.getElementById('cart-count'); if(!cartCountEl) return; const totalItems = cart.reduce((sum, item) => sum + item.quantity, 0); if (totalItems > 0) { cartCountEl.textContent = totalItems; cartCountEl.style.display = 'block'; } else { cartCountEl.style.display = 'none'; } }
  function renderCartPage() { const cart = getCart(); const cartItemsContainer = document.getElementById('cart-items-container'); const cartSummaryContainer = document.getElementById('cart-summary-container'); if (!cartItemsContainer) return; cartItemsContainer.innerHTML = ''; if (cart.length === 0) { cartItemsContainer.innerHTML = '<p>Keranjang belanja Anda masih kosong.</p>'; if (cartSummaryContainer) cartSummaryContainer.style.display = 'none'; return; } else { if (cartSummaryContainer) cartSummaryContainer.style.display = 'block'; } const totalKpQuantity = getTotalQuantityForPrefix(cart, "KP"); let totalPrice = 0; cart.forEach(item => { const currentPrice = getTieredPrice(item, totalKpQuantity); totalPrice += currentPrice * item.quantity; let priceHtml = '<div class="cart-item-price">Rp ' + currentPrice.toLocaleString('id-ID') + '</div>'; if (currentPrice < item.price) { priceHtml = '<div class="cart-item-price discounted">Rp ' + currentPrice.toLocaleString('id-ID') + '</div>' + '<div class="cart-item-price original-price"><s>Rp ' + item.price.toLocaleString('id-ID') + '</s></div>'; } const itemEl = document.createElement('div'); itemEl.className = 'cart-item'; itemEl.innerHTML = ` <img src="${item.image}" alt="${item.name}" class="cart-item-img"> <div class="cart-item-details"> <div class="cart-item-name-wrapper"><div class="cart-item-name">${item.name}</div></div> <div class="cart-item-price-wrapper">${priceHtml}</div> </div> <div class="cart-item-quantity"> <button class="quantity-btn" onclick="updateQuantity('${item.id}', -1)">-</button> <span>${item.quantity}</span> <button class="quantity-btn" onclick="updateQuantity('${item.id}', 1)">+</button> </div> `; cartItemsContainer.appendChild(itemEl); }); document.getElementById('total-price').textContent = 'Rp ' + totalPrice.toLocaleString('id-ID'); document.getElementById('discount-row').style.display = 'none'; const voucherInput = document.getElementById('voucher-input'); if (voucherInput) voucherInput.value = ''; }
  let appliedVoucher = null;
  function applyVoucher() { const voucherInput = document.getElementById('voucher-input'); const voucherCode = voucherInput.value.toUpperCase(); const cart = getCart(); if (VOUCHER_CODES[voucherCode]) { const voucher = VOUCHER_CODES[voucherCode]; let totalDiscount = 0; cart.forEach(item => { if (item && typeof item.id === 'string' && item.id.startsWith(voucher.target)) { totalDiscount += voucher.discount * item.quantity; } }); if (totalDiscount > 0) { appliedVoucher = voucherCode; document.getElementById('discount-amount').textContent = '- Rp ' + totalDiscount.toLocaleString('id-ID'); document.getElementById('discount-row').style.display = 'flex'; const subtotal = cart.reduce((sum, item) => sum + (getTieredPrice(item, getTotalQuantityForPrefix(cart, "KP")) * item.quantity), 0); const finalTotal = subtotal - totalDiscount; document.getElementById('total-price').textContent = 'Rp ' + finalTotal.toLocaleString('id-ID'); showNotificationPopup('Yay! Voucher berhasil digunakan!'); } else { showNotificationPopup('Voucher ini tidak berlaku untuk produk di keranjangmu.'); appliedVoucher = null; } } else { showNotificationPopup('Kode voucher salah atau tidak valid.'); appliedVoucher = null; } }
  window.updateQuantity = (productId, change) => { let cart = getCart(); const item = cart.find(i => i.id === productId); if (item) { item.quantity += change; if (item.quantity <= 0) { cart = cart.filter(i => i.id !== productId); } } saveCart(cart); renderCartPage(); };
  const applyVoucherBtn = document.getElementById('apply-voucher-btn'); if (applyVoucherBtn) { applyVoucherBtn.addEventListener('click', applyVoucher); }
  const closeNotificationBtn = document.getElementById('close-notification-btn'); if (closeNotificationBtn) { closeNotificationBtn.addEventListener('click', () => { const popup = document.getElementById('notification-popup'); if(popup) popup.style.display = 'none'; }); }
  function initializeNavMenu() { const hamburgerMenu = document.querySelector('.hamburger-menu'); const closeBtn = document.querySelector('.close-btn'); const dropdownBtn = document.querySelector('.dropdown-btn'); if (hamburgerMenu) { hamburgerMenu.addEventListener('click', () => { document.getElementById('mySidenav').style.width = '250px'; }); } if (closeBtn) { closeBtn.addEventListener('click', () => { document.getElementById('mySidenav').style.width = '0'; }); } if (dropdownBtn) { dropdownBtn.addEventListener('click', function() { this.classList.toggle('active'); const dropdownContent = this.nextElementSibling; if (dropdownContent) dropdownContent.style.display = dropdownContent.style.display === 'block' ? 'none' : 'block'; }); } }
  function initializeImageModal() { const modal = document.getElementById('imageModal'); const modalImg = document.getElementById('modalImage'); const closeSpan = document.querySelector('.modal-close'); if (!modal || !modalImg || !closeSpan) return; document.body.addEventListener('click', function(event) { if (event.target.classList.contains('product-image')) { modal.style.display = 'block'; modalImg.src = event.target.src; } }); closeSpan.onclick = () => { modal.style.display = 'none'; }; window.onclick = (event) => { if (event.target == modal) modal.style.display = "none"; } }
  let toastTimeout; function showToast(message) { const toast = document.getElementById('toast-notification'); if (!toast) return; toast.textContent = message; toast.classList.add('show'); clearTimeout(toastTimeout); toastTimeout = setTimeout(() => { toast.classList.remove('show'); }, 1500); }
  function showNotificationPopup(message) { const popup = document.getElementById('notification-popup'); const messageEl = document.getElementById('notification-message'); if (popup && messageEl) { messageEl.textContent = message; popup.style.display = 'flex'; } }
  function showPromoPopup() { const popup = document.getElementById('promo-popup'); if (popup) popup.style.display = 'flex'; }
  function initializePromoPopup() { const promoPopup = document.getElementById('promo-popup'); if (!promoPopup) return; setTimeout(showPromoPopup, 1000); const closePromoBtn = document.getElementById('close-promo-popup'); if (closePromoBtn) { closePromoBtn.addEventListener('click', () => promoPopup.style.display = 'none'); promoPopup.addEventListener('click', (event) => { if (event.target === promoPopup) promoPopup.style.display = 'none'; }); } }
  window.showSizeChart = (imageUrl) => { const modal = document.getElementById('imageModal'); const modalImg = document.getElementById('modalImage'); if (modal && modalImg) { modalImg.src = imageUrl; modal.style.display = 'block'; } }
